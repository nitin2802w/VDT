...
@dataclass
class PendingSymbol:
    """A buffered symbol that still combines two or more unknown blocks."""
...
class PeelingDecoder:
    ...
    def add_symbol(self, seed: int, data: bytes) -> bool:
        """Feed one received symbol (seed + its XORed payload) into the decoder."""
    ...
    def get_reconstructed_blocks(self) -> List[bytes]:
        """Return all k blocks in order. Raises if decoding isn't complete yet."""
